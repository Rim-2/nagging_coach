...
| 파일 | 역할 |
|---|---|
| `app.py` (875줄) | `CoachApp` 본체 — `__init__` · `_on_update` · chat/photo 핸들러 · /reset 확인 쿠션 · chat debounce · 잔소리 버튼·스누즈 · main 진입점 |
| `app_http.py` (159줄) | `TriggerHTTPHandler` + `HttpServerMixin` — `POST /trigger` · `GET /health` · `GET /weak_spots` · `GET /places` |
| `app_messaging.py` (300줄) | `MessagingMixin` — `_send` (짧은 backoff) · `_send_or_enqueue` · `_apply_message_side_effects` · retry 큐 워커 · 큐 모니터링 알림 · 톤 노트 |
| `app_triggers.py` (453줄) | `TriggersMixin` — `_on_trigger` (로컬) · `handle_remote_trigger` (원격) · `_describe_trigger` · 복합 룰 평가 · 메타 체크인 |
| `app_loops.py` (408줄) | `LoopsMixin` — proactive · reminder · alarm · risk_predict · daily_journal · weekly_review · daily_restart 데몬 + 헬퍼 |
| `presence.py` (109줄) | `Presence` — 수면/기상/활동 추론의 단일 주인 (밤잠 판정·야간 가드·기기 활동 신호 + 관련 임계 상수 한 곳). app/loops/triggers 가 위임 |
...
| `tests/` | pytest 단위 테스트 (90개) — store/tracker/복합 트리거/잔소리 응답·메타 체크인·무시 대응·밤잠 추론·성취 보상 |
...
- **밤잠 추론** — 폰이 **화면 OFF 지속시간(`screen_off_sec`)을 기존 트리거 페이로드에 동봉**(별도 heartbeat 없이 → 배터리 비용 0). 백엔드는 이 값을 권위 신호로 써 *자다 깸*(직전까지 화면 OFF 1.5h+)과 *폰 하느라 안 잠*(화면 계속 ON)을 정확히 구분한다. 폰 신호가 없으면(구버전 APK) **기기 활동(트리거) 시각** 휴리스틱으로 폴백:
  - 긴 메시지 침묵(3~16h) + 아침 + *기기도 잠잠* → 자다 깸 후보 → 단정 말고 "자다 일어난 거야?" 하고 **묻는다**
...
- 단위 테스트 90개 (store · tracker · 복합 트리거 · 잔소리 응답 · 메타 체크인 · 무시 대응 · 밤잠 추론 · 성취 보상)
