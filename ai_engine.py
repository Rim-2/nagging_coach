...
- "new_today_goals": 사용자가 '오늘 할 일'(단발성 과제)을 새로 말했으면 그
  목표들을 리스트로 (여러 개면 여러 개). 꾸준히 하는 습관은 제외 — 그건
  register_habit 도구가 따로 다룬다. [현재 저장된 상태]에 이미 있는 목표도
  제외. 없으면 [].
- "completed_today_goals": 오늘 목표 중 완료된 게 있으면 리스트로. 단 '완료'는
  코치 답장이 실제로 완료를 인정·축하한 경우만 — 코치가 "인증샷 보내봐" 하고
  증거를 요구한 상태면 아직 완료가 아니다. [현재 저장된 상태]의 목표 표현에
  맞춰서. 없으면 [].
...
class CoachAgent:
    ...
    def __init__(
        ...
        *,
        ...
        self._tools = build_tools(calendar, store)  # {name: Tool}
    ...
    def _apply_extracted(self, data: dict) -> None:
        ...
        if isinstance(new_goals, list):
            for g in new_goals:
                g = str(g).strip()
                if g and store.add_today_goal(g):
                    print(f"[CoachAgent] 오늘 목표 추가: {g}")
                    self._on_goal_set(g)
        ...
        if isinstance(done_goals, list):
            ...
            for g in done_goals:
                g = str(g).strip()
                if g and store.complete_today_goal(g):
                    print(f"[CoachAgent] 오늘 목표 완료: {g}")
                    removed = True
